...
class DatabricksStack(core.Stack):
    ...
    def __init__(self, scope: core.Construct, **kwargs) -> None:
        ...
        cross_account_role = iam.Role(
            self,
            id=f"iam-{self.deploy_env.value}-databricks-cross-account-role", # Após criado é colocado no databricks
        ...
        cross_account_policy = iam.Policy(
            ...
            statements=[
                ...
                iam.PolicyStatement(
                    ...
                    resources=[
                        f"arn:aws:s3:::s3-bck-{self.deploy_env.value}-data-lake-*",
                ...
                ), # Permissoes pra usar a autoloader da databricks
                iam.PolicyStatement(
        ...
        # Preciso criar um bucket pro data 
        bucket = s3.Bucket(
            self,
            id=f"s3-bck-{self.deploy_env.value}-databricks",
            bucket_name=f"s3-bck-{self.deploy_env.value}-databricks",
        )
        bucket.add_to_resource_policy(
            iam.PolicyStatement(
                # Política que deixará a conta da databricks (account_id) fazer as operações dentro do bucket.
        ...
        access_policy = iam.Policy(
            ...
            statements=[
                iam.PolicyStatement(
                    ...
                    resources=[
                        # Hard code pra agilizar, mas idealmente importaríamos o nome do bucket e usaríamos o objeto como temos feito.
                        f"arn:aws:s3:::s3-bck-{self.deploy_env.value}-data-lake-*",
                        f"arn:aws:s3:::s3-bck-{self.deploy_env.value}-data-lake-*/*",
                    ],
                ),
        ...
        iam.CfnInstanceProfile( # Adiciona esta intance profile no cluster do databricks
            self,
